import type { Metadata } from "next";
import { notFound } from "next/navigation";

import {
  BrowseNav,
  CurriculumBreadcrumbs,
  CurriculumHero,
  DisciplineSection,
  EmptyState,
  ProgressNotice
} from "@/features/curriculum/components";
import { getCurriculum, getSchool } from "@/features/curriculum/data";

export const metadata: Metadata = {
  title: "Future Engineering | Industrial Learn"
};

export default function FutureEngineeringPage() {
  const curriculum = getCurriculum();
  const school = getSchool("future-engineering");

  if (!school) {
    notFound();
  }

  return (
    <div className="curriculum-page">
      <BrowseNav pathways={curriculum.pathways} schools={curriculum.schools} />
      <CurriculumBreadcrumbs
        items={[{ href: "/learn/future-engineering", label: school.title }]}
      />
      <CurriculumHero
        description={school.description}
        eyebrow="School"
        title={school.title}
      />
      <ProgressNotice />
      {school.disciplines.length === 0 ? (
        <EmptyState message="No Future Engineering disciplines are available yet." />
      ) : (
        school.disciplines.map((discipline) => (
          <DisciplineSection discipline={discipline} key={discipline.id} />
        ))
      )}
    </div>
  );
}
